...
"""
This module implements the trigger of "ctrl+up".
"""
...
class CtrlUp(object):
    ...
    def activate(self):
        ...
        if self.buffer.get_has_selection():
            ...
            self.up_iters[0] = self.current_iters[0].copy()
            self.up_iters[1] = self.current_iters[0].copy()
            self.up_iters[0].backward_line()
            self.up_iters[1].backward_line()
        ...
        if not self.current_iters[0].is_start():
            ...
            self.buffer.delete(self.up_iters[0], self.current_iters[1])
            
            if select_flag:
                insert_mark = self.buffer.get_insert()
                insert_iter = self.buffer.get_iter_at_mark(insert_mark)
                mark_left = self.buffer.create_mark("left", insert_iter, True)
            ...
            if select_flag:
                ...
                mark_right = self.buffer.create_mark("right", aux_iter, True)
